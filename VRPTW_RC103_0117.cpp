...
//-----------------------------------input--------------------------
    ...
    input_text_data.open("RC103.txt");
    while(1){
...
//-----------------------------------------------最適化-----------------------------
    ...
    while((double)(end-start)/CLOCKS_PER_SEC<3){
        ...
        if(in_or_out==0){//-------------------------経路内--------------------
            ...
            for(int i=1;i<new_root1.size();i++){
                int now = new_root1[i-1],nex = new_root1[i];
                if(t+dist[now][nex]>due_time[nex]||cp+demand[i]>max_capacity){
                    flag=1;
                    break;
                }
                new_dist1+=dist[now][nex];
                v_t1[i]=t+dist[now][nex];
                if(t+dist[now][nex]<=ready_time[nex]) t=ready_time[nex] + service_time[nex];
                else t+=dist[now][nex] + service_time[nex];
            }
            t = 0;
            cp = 0;
            if(flag == 0){
                for(int i=1;i<new_root2.size();i++){
                    int now = new_root2[i-1],nex = new_root2[i];
                    if(t+dist[now][nex]>due_time[nex]||cp+demand[i]>max_capacity){
                        flag=1;
                        break;
                    }
                    new_dist2+=dist[now][nex];
                    v_t2[i]=t+dist[now][nex];
                    if(t+dist[now][nex]<=ready_time[nex]) t=ready_time[nex] + service_time[nex];
                    else t+=dist[now][nex] + service_time[nex];
                }
            }
...
//---------------------------------書き出し------------------------------
    ...
    output_text_data.open("RC103_c4_output.csv");
    rep(i,max_car){
        if(root[i].size()>1){
            rep(j,root[i].size()){
