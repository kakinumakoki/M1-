...
//------------------------------------初期解-------------------------------------
    ...
    while(i<max_car){//制限時間内の一つの車の行動決めてから、次の車
        ...
        while(t<limit_time){
            ...
            next=-1;
            for(int j=1;j<N;j++){
                cost = 0.2*dist[now_point][j]+0.1* (due_time[j] - dist[now_point][j]-t)+ 0.6*min_0(ready_time[j]-t);
                if(arrived[j] == false&&ok_capacity(capacity,j)&&t+dist[now_point][j]<=due_time[j]&&min_cost>cost &&now_point!=j){
                    min_cost = cost;
                    next = j;
                }
            }
            if(t+dist[now_point][0]>=limit_time){
                ...
                if(t-ready_time[next]<0) cout<<t-ready_time[next]<<endl;
